        ...
        <form action="{{ route('reminder.update', [$contact->id, $reminder->id]) }}" method="POST">
            ...
            <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control{{ $errors->has('title') ? ' is-invalid' : '' }}" value="{{ $reminder->title }}" name="title" id="title" placeholder="I want to be reminded of . . .">
                </div>

                <div class="mb-3">
                    <label for="happened_at">Choose a date</label>
                    <input type="text" class="form-control{{ $errors->has('reminder_date') ? ' is-invalid' : '' }} datepicker-reminder" value="@if (!empty( $reminder->reminder_date )) {{ date('j F Y', strtotime( $reminder->reminder_date )) }} @endif" name="reminder_date" id="reminder_date" placeholder="Choose a date">
                </div>
